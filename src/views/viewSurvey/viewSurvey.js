...
export default function SurveyPage() {
  ...
  const [numOfscenarios, setnumOfscenarios] = useState("");
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      data = await api.user.viewSurvey({
        surveyId: survey_id,
      });
      console.log(data)
      if (data[1]["isArchived"] === true) {
        history.push("designer/survey/all");
        toast.error("Survey is archived", {
          position: "top-right",
          autoClose: 5000,
        ...
        setDescription1(data[1].description.description1);
        setDescription2(data[1].description.description2);
        setDescription3(data[1].description.description3);
        setTitle(data[1].title);
        setSurveyType(data[1].surveyType);
        setnumOfoptions(data[1].numofOptions);
        setnumOfscenarios(data[1].numofScenario);
        setattributeData(data[1].option);
        setusrData(data[1].userDetails);
        setimageData1(data[1].images[0]);
        setimageData2(data[1].images[1]);
        setimageData3(data[1].images[2]);
      }
    }
    fetchData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // const uploadImage = (urldata,binaryBlob) => {
  //   axios({
  //     method: "put",
  //     url: urldata,
  //     data: binaryBlob,
  ...
  //       "Key": "test.png"
  //   });
  //   await uploadImage(data.url, binaryBlob)
  // }
  ...
  const SubmitData = async () => {
    ...
    let res;
    let data;
    [res, data] = await api.designer.updateSurvey(survey);
    if (res.status === 200) {
      ...
      toast.error("Failed updating the survey", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  return (
    <div className="p-2">
      ...
      <Steps current={step}>
        ...
        <Steps.Item
          title="Social Economic Information"
      ...
      <Panel hidden={(step === 1) | (step === 2) | (step === 3)}>
        <div class="grid grid-flow-row auto-rows-max pb-3">
          ...
          <div class="mb-3">
            <div>
              <TextArea
                moduleVal={moduleVal_Title}
                txtvalue={title}
                txtsetValue={setTitle}
              />
            </div>
          </div>
        </div>
        <div class="grid grid-flow-row auto-rows-max pb-3">
          <div>
            <h5>Description for first page</h5>
          </div>
          <div>
            <TextArea
              moduleVal={moduleVal}
              txtvalue={description1}
              txtsetValue={setDescription1}
            />
          </div>
        </div>
        <div class="grid grid-flow-row auto-rows-max pb-3">
          <div>
            <h5>Images for the first page</h5>
          </div>
        ...
        <div class="grid grid-flow-row auto-rows-max pb-3">
          <div>
            <h5>Description for second page</h5>
          </div>
          <div>
            <TextArea
              moduleVal={moduleVal}
              txtvalue={description2}
              txtsetValue={setDescription2}
            />
          </div>
        </div>
        <div class="grid grid-flow-row auto-rows-max pb-3">
          <div>
            <h5>Images for the second page</h5>
          </div>
          <div><p>If Multiple photos are uploaded upload as one pictures after creating a collage, else pictures will be shown below each</p></div>
        ...
        <div class="grid grid-flow-row auto-rows-max pb-3">
          <div>
            <h5>Description for third page</h5>
          </div>
          <div>
            <TextArea
              moduleVal={moduleVal}
              txtvalue={description3}
              txtsetValue={setDescription3}
            />
          </div>
        </div>
        <div class="grid grid-flow-row auto-rows-max pb-3">
          <div>
            <h5>Images for the third page</h5>
          </div>
          <div><p>If Multiple photos are uploaded upload as one pictures after creating a collage, else pictures will be shown below each</p></div>
      ...
      <Panel hidden={(step === 2) | (step === 3) | (step === 0)}>
        <div>
          <div class="mb-6">
            ...
            >
              <option value="BEST_CHOICE" selected>
                Choice Survey
              </option>
          ...
          <div class="mb-6">
            <label
              ...
              class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
            >
              Number of Games per survey
            </label>
            <input
              type="number"
              id="email"
              value={numOfscenarios}
              onChange={(e) => setnumOfscenarios(e.target.value)}
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder="Add the number of games for each survey"
              required
            />
          </div>
          <div class="mb-6">
      ...
      <div class="flex justify-center m-2 p-6">
        ...
        {step === 3 ? 
          <CButton
            className="m-2 w-40"
            onClick={SubmitData}
            disabled={validData === false || onsubmission}
            color="success"
            size="sm"
          >
            {"Update"}
            ...
            className="m-2 w-40"
            onClick={onNext}
            disabled={step === 3 || validData === false}
            color="primary"
            size="sm"
          >
            {"Next"}
